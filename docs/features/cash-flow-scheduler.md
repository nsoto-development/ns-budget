...
| Design system | **nsoto design-system** | Brand alignment with portfolio; **flesh out consumption before M2 UI** (see open questions) |
...
| M2 | App scaffold + P0 UX | Planned | SvelteKit scaffold; **design-system consumption agreed first**; income/bill config lists; starting balance + horizon; line chart (deficit in red); summary stats wired to engine (client-side preview). |
| M3 | Plan persistence | Planned | Plan data model; save/load API; Neon on Vercel; anonymous shareable plan IDs. |

**Next pass:** M2 — app scaffold + P0 UX.
...
- Add framework lint/test commands to `nudl.json` `verify.commands` at M2 scaffold.

## Open questions

- **Design-system consumption (before M2 UI):** `@nsoto/tokens` only vs. Svelte wrappers / hand-rolled components guided by ui_kits — `@nsoto/ui` is React-oriented; decide approach then update this table.
